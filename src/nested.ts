...
export function sumPoints(questions: Question[]): number {
    //return questions.map((question) => question.points).sum();
...
export function sameType(questions: Question[]): boolean {
    ...
    return questions.find((question) => question.type !== type) === undefined;
}
...
export function addNewQuestion(
    ...
    const out = [...questions];
    out.push(makeBlankQuestion(id, name, type));
    return out;
}
...
export function duplicateQuestionInArray(
    ...
    const originalIndex = questions.indexOf(original);
    //Add in copy into newlist at index originalIndex + 1
    newList.splice(originalIndex + 1, 0, copy);
    return newList;
}
